...
EOF
    ...
    if [ ! -d "/usr/src/trojan-cert" ]; then
        ...
        ~/.acme.sh/acme.sh --register-account -m z123zxc2@foxmail.com  --issue  -d $your_domain  --nginx
...
EOF
        ...
        cd /root
        ~/.acme.sh/acme.sh --register-account -m z123zxc2@foxmail.com --installcert  -d  $your_domain   \
...
function repair_cert(){
    ...
    local_addr=`curl ipv4.icanhazip.com`
    if [ $real_addr == $local_addr ] ; then
        ~/.acme.sh/acme.sh  --register-account -m z123zxc2@foxmail.com --issue  -d $your_domain  --standalone
        ~/.acme.sh/acme.sh  --register-account -m z123zxc2@foxmail.com --installcert  -d  $your_domain   \
            --key-file   /usr/src/trojan-cert/$your_domain/private.key \
            --fullchain-file /usr/src/trojan-cert/$your_domain/fullchain.cer \
            --reloadcmd  "systemctl restart trojan"
